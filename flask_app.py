    ...
    while True:
        ...
        if not success:
            ...
            for obj in coords:
                ...
                x2 = obj.box[2]
                y2 = obj.box[3]
                x_1, y_1, x_2, y_2 = define_world_space_position(x1, y1, x2, y2)
                x_1 = int(x_1)
                x_2 = int(x_2)
                y_1 = int(y_1)
                y_2 = int(y_2)
                moving_obstacles += construct_shape((x_1, y_1), (x_2, y_2), (x_1, y_2), (x_2, y_1))
                for obs in moving_obstacles:
...
def define_world_space_position(x1, y1, x2, y2):
    ...
    resolution_width = int(camera.get(cv2.CAP_PROP_FRAME_WIDTH))  # Width resolution of the camera
    resolution_height = int(camera.get(cv2.CAP_PROP_FRAME_HEIGHT))  # Height resolution of the camera
    ...
    angle_offset_horizontal = (box_center_x - resolution_width / 2) * pixel_size_horizontal
    angle_offset_vertical = (box_center_y - resolution_height / 2) * pixel_size_vertical

    # Convert the angle offsets to real-world distances
    distance_horizontal = math.tan(math.radians(angle_offset_horizontal)) * grid_width / 2
    distance_vertical = math.tan(math.radians(angle_offset_vertical)) * grid_height / 2

    # Adjust the angle offsets based on the camera's rotation angle
    adjusted_angle_offset_horizontal = math.radians(camera_rotation) + math.radians(angle_offset_horizontal)
    adjusted_angle_offset_vertical = math.radians(camera_rotation) + math.radians(angle_offset_vertical)

    # Convert the adjusted angle offsets to real-world distances
    adjusted_distance_horizontal = math.tan(adjusted_angle_offset_horizontal) * grid_width / 2
    adjusted_distance_vertical = math.tan(adjusted_angle_offset_vertical) * grid_height / 2
    ...
    bottom_left_x = camera_x + distance_horizontal - (box_width_real / 2) + adjusted_distance_horizontal
    bottom_left_y = camera_y + distance_vertical - (box_height_real / 2) + adjusted_distance_vertical
